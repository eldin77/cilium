...
func fetchK8sLabels(containerID string, containerLbls map[string]string) (map[string]string, error) {
	...
	if isInjectedWithSidecarProxy(containerID, result) {
		k8sLabels[k8sConst.PolicyLabelIstioSidecarProxy] = ""
	}

	return k8sLabels, nil
}

// isInjectedWithSidecarProxy returns whether the given pod has been injected
// with an Istio sidecar proxy that is compatible with Cilium.
func isInjectedWithSidecarProxy(containerID string, pod *corev1.Pod) bool {
